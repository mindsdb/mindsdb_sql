...
class TestPlanEvaluate:
    def test_plan_evaluate(self):
        query = Evaluate(
            ...
            using=None,
            query_str="""SELECT t.column1, m.predicted FROM int.tab1 AS t JOIN mindsdb.pred AS m LIMIT 100"""
        )
            ...
            steps=[
                FetchDataframeStep(integration='int',
                                   query=Select(targets=[Star()],
                                                from_table=Identifier('tab1', alias=Identifier('t')),
                                                limit=Constant(100)),
                                   ),
                ApplyPredictorStep(namespace='mindsdb',
                                   dataframe=Result(0),
